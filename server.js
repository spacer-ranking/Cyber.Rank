...
function initDatabase() {
    db.serialize(() => {
        ...
        // Таблица команд (leader_id может быть NULL)
...
app.post('/api/teams/:teamId/set-leader', authenticateToken, isLeader, (req, res) => {
    ...
    db.get("SELECT * FROM teams WHERE id = ?", [teamId], (err, team) => {
        ...
        if (team.leader_id) {
            return res.status(400).json({ error: 'У команды уже есть лидер' });
        }
        
        // Назначаем текущего пользователя лидером
        db.run("UPDATE teams SET leader_id = ? WHERE id = ?", 
            [req.user.id, teamId], 
            function(err) {
                if (err) return res.status(500).json({ error: 'Ошибка назначения лидера' });
                
                console.log(`✅ Лидер ${req.user.username} назначен для команды ${team.name}`);
                res.json({ success: true, message: 'Лидер назначен' });
            });
    });
});
