...
// Создание команды (без автоматического добавления лидера)
    ...
    db.get("SELECT * FROM teams WHERE name = ?", [name], (err, team) => {
        ...
        db.run("INSERT INTO teams (name, leader_id, rating) VALUES (?, ?, 0)",
            ...
            function(err) {
                ...
                // Лидер НЕ добавляется в team_members автоматически
...
// Добавление игрока (Quantum не становится лидером)
app.post('/api/teams/:teamId/members', authenticateToken, isLeader, (req, res) => {
    ...
    db.get("SELECT * FROM teams WHERE id = ? AND leader_id = ?", [teamId, req.user.id], (err, team) => {
        ...
        // Проверяем, не пытается ли лидер добавить себя
        if (member_name === req.user.username) {
            return res.status(400).json({ error: 'Вы уже являетесь лидером этой команды' });
        }
