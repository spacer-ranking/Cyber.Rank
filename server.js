...
// Настройка почтового транспорта с улучшенным логированием
...
const transporter = nodemailer.createTransport({
    ...
    auth: {
        ...
        pass: 'ваш_пароль_приложения' // ЗАМЕНИТЕ НА СВОЙ ПАРОЛЬ ПРИЛОЖЕНИЯ
    },
    debug: true, // Включаем отладку
    logger: true // Логируем все действия
});
...
app.post('/api/forgot-password', async (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE email = ?", [email], async (err, user) => {
        ...
        try {
            ...
            // Отправляем код в ответе для тестирования (только для разработки)
            res.status(500).json({ 
                error: 'Ошибка отправки email. Проверьте консоль сервера для получения кода.',
                debug: { code: resetCode } // Временное решение для отладки
            });
        }
    });
});
