...
function setupEventListeners() {
    ...
    if (closeForgotBtn) {
        closeForgotBtn.addEventListener('click', () => hideModal(forgotPasswordModal));
    }
            ...
            <div class="section-header">
                ...
                ${currentUser?.role === 'leader' ? `
                    <div class="leader-actions">
                        <button class="leader-btn" onclick="window.showCreateTeamModal()">+ Создать команду</button>
                        <button class="leader-btn" onclick="window.showCreateMatchModal()">+ Создать матч</button>
                    </div>
                ` : ''}
            </div>
                ...
                html += `
                    <div class="match-card" id="match-${match.id}">
                        ...
                        ${currentUser?.role === 'leader' && match.created_by === currentUser?.id ? `
                            <button class="delete-match-btn" onclick="window.deleteMatch(${match.id})">Удалить матч</button>
                        ` : ''}
                    </div>
                `;
            });
...
async function showTeam(teamId) {
    try {
        ...
        // Если пользователь лидер и команда без лидера, показываем кнопку "Стать лидером"
        if (currentUser?.role === 'leader' && !team.leader_name) {
            html += `
                <div class="leader-actions" style="margin-top: 1rem;">
                    <button class="leader-btn" onclick="window.setAsLeader(${team.id})">👑 Стать лидером команды</button>
                </div>
            `;
        }

        // Если пользователь является лидером этой команды
        if (currentUser?.role === 'leader' && currentUser?.username === team.leader_name) {
            html += `
                <div class="leader-actions" style="margin-top: 1rem; display: flex; gap: 0.5rem; flex-wrap: wrap;">
                    ...
                    <button class="leader-btn" style="background: #dc3545;" onclick="window.deleteTeam(${team.id})">🗑 Удалить команду</button>
                </div>
            `;
        }
        ...
        if (team.members && team.members.length > 0) {
            team.members.forEach(member => {
                ...
                html += `
                    <div class="member-card ${isLeader ? 'leader-card' : ''}" id="member-${member.id}">
                        ...
                        <div class="member-info">
                            <div class="member-name-row">
                                ...
                                ${isLeader ? '<span class="leader-badge-small">👑 ЛИДЕР</span>' : ''}
                            </div>
...
async function setAsLeader(teamId) {
    if (!confirm('Вы хотите стать лидером этой команды?')) return;
    
    try {
        const response = await fetch(`/api/teams/${teamId}/set-leader`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({})
        });
        ...
        if (response.ok) {
            alert('Теперь вы лидер этой команды!');
            showTeam(teamId);
        } else {
            alert(data.error || 'Ошибка назначения лидера');
        }
    } catch (error) {
...
async function changeMemberRole(teamId, memberId, newRole) {
    try {
        ...
        if (response.ok) {
            const roleSpan = document.querySelector(`#member-${memberId} .member-role`);
...
window.setAsLeader = setAsLeader;