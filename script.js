...
async function sendResetCode() {
    ...
    if (successEl) {
        successEl.textContent = 'Отправка кода...';
        successEl.style.display = 'block';
        successEl.style.color = '#ffff00';
    }
    
    try {
        console.log('Отправка запроса на восстановление для email:', email);
        ...
        if (response.ok) {
            ...
            if (successEl) {
                successEl.textContent = 'Код отправлен на ваш email!';
                successEl.style.display = 'block';
                successEl.style.color = '#00ff00';
            ...
            if (data.debugCode) {
                console.log('🔐 Код для отладки:', data.debugCode);
                if (successEl) {
                    successEl.textContent = `Код: ${data.debugCode} (скопируйте его)`;
                    successEl.style.display = 'block';
                    successEl.style.color = '#ffff00';
                }
            }
...
async function verifyResetCode() {
    ...
    if (!resetEmail) {
        if (errorEl) {
            errorEl.textContent = 'Ошибка: email не найден. Начните заново.';
            errorEl.classList.add('show');
        }
        return;
    }
    ...
    try {
        console.log('Проверка кода:', { email: resetEmail, code });
        ...
        if (response.ok) {
            ...
            if (successEl) {
                successEl.textContent = 'Код подтвержден! Введите новый пароль.';
                successEl.style.display = 'block';
                successEl.style.color = '#00ff00';
            }
        } else {
...
async function resetPassword() {
    ...
    if (!resetEmail || !resetCode) {
        if (errorEl) {
            errorEl.textContent = 'Ошибка: начните процесс восстановления заново';
            errorEl.classList.add('show');
        }
        return;
    }
    ...
    try {
        console.log('Сброс пароля для:', resetEmail);
        ...
        if (response.ok) {
            if (successEl) {
                successEl.textContent = 'Пароль успешно изменен! Сейчас вы будете перенаправлены на вход.';
                successEl.style.display = 'block';
                successEl.style.color = '#00ff00';
            }
